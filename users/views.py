...
from .forms import \
    UserRegisterForm, UserUpdateForm, ProfileUpdateForm, ProfileViewForm, ProfileUpdateFormRecruiter, ProfileViewFormRecruiter
...
def profile(request):
    if ( request.user.profile.is_recruiter == False) :
        ...
        if request.method == 'POST':
            u_form = UserUpdateForm(request.POST, instance=request.user)
            p_form = ProfileUpdateFormRecruiter(request.POST,
                                       request.FILES,
                                       instance=request.user.profile)
            if u_form.is_valid() and p_form.is_valid():
                u_form.save()
            ...
            u_form = UserUpdateForm(instance=request.user)
            p_form = ProfileUpdateFormRecruiter(instance=request.user.profile)
            v_form = ProfileViewFormRecruiter(instance=request.user.profile)
